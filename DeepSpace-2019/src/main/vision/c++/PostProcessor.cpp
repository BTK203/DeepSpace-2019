...
void PostProcessor::Loop()
{
    ...
    while (!stop)
    {
        ...
        if (readSuccess)
        { //only continue processing if we could actually read the image
            ...
            for (int i = 0; i < contours.size(); i++)
            {
                ...
                if (Util::IsElgible(minRect))
                { //the contour is elgible to be part of a target!
                    if (Settings::DEBUG)
                    {
                        cv::Rect boxRect = minRect.boundingRect(); //draw the bounding rect on the image
                    ...
                    for (int a = 0; a < unpairedRects.size(); a++)
                    {
                        ...
                        if (Util::IsPair(minRect, otherRect))
                        { //ladies and gentlemen, we got em
                            ...
                            if (pair.area() > biggestTarget.area() && !Settings::DOCK_USING_CLOSEST_TO_CENTER)
                                biggestTarget = pair; // if biggest area differentiation is enabled, take the biggest target

                            if (Settings::DOCK_USING_CLOSEST_TO_CENTER)
                            {
                                if (pair.distanceFromCenter() < biggestTarget.distanceFromCenter() || !targetFound)
                                {
                                    biggestTarget = pair; //if closest target differentiation is enabled (recommended) then take the target that is closest to center
                                    targetFound = true;
                                }
                            }
                        }
            ...
            if (target_x < 0)
            { //lock up because we cannot see a target
                locked = true;
            ...
            if (locked)
            {
                ...
                if (target_x > -1)
                { //there is indeed a target in view
                    lock_frames++;
